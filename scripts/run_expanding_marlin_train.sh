#!/bin/bash
# Submit a committed EFlow/EFM run from an isolated detached worktree.
...
WORKTREE_ROOT="$REPRO_ROOT/experiment-worktrees"
WORKTREE="$WORKTREE_ROOT/expanding-${COMMIT:0:12}"
...
if [[ "$WORKTREE_COMMIT" != "$COMMIT" ]]; then
    echo "Expanding worktree commit mismatch: $WORKTREE_COMMIT != $COMMIT" >&2
    exit 2
fi
flock -u 9
...
sbatch \
    ...
    scripts/slurm_expanding_marlin_train.sbatch

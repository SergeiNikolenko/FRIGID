...
EVALUATION_FINGERPRINTS_REL="${MARLIN_EVALUATION_FINGERPRINTS_REL:-val/fingerprints.npz}"
EVALUATION_FINGERPRINT_KEY="${MARLIN_EVALUATION_FINGERPRINT_KEY:-ground_truth}"
EVALUATION_FINGERPRINT_THRESHOLD="${MARLIN_EVALUATION_FINGERPRINT_THRESHOLD:-}"
...
RESUME_CHECKPOINT_STEP="${MARLIN_RESUME_CHECKPOINT_STEP:-}"
...
if [[ -n "$RESUME_CHECKPOINT_TASK_ID" ]]; then
  ...
  test "$RESUME_CHECKPOINT_STEP" -ge 0
  RESUME_CHECKPOINT="$(
    python scripts/materialize_marlin_checkpoint.py \
      ...
      --output-name "step=$RESUME_CHECKPOINT_STEP.ckpt"
  )"
fi
...
python scripts/train_marlin.py \
  ...
  evaluation.fingerprints="$RUNTIME_ROOT/$EVALUATION_FINGERPRINTS_REL" \
  evaluation.fingerprint_key="$EVALUATION_FINGERPRINT_KEY" \
  evaluation.threshold="$EVALUATION_FINGERPRINT_THRESHOLD" \

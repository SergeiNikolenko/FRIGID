...
def _bundle(
    ...
    *,
    ...
    if end_to_end or autoresearch or adaptation:
        payloads.update(
            {
    ...
    if adaptation:
        payloads.update(
            {
                "train/dreams_predictions.npz": b"train predictions",
                "train/dreams_predictions.summary.json": b"{}",
                "train/metadata.csv": b"spec_name,smiles\ntrain-a,CC\n",
            }
        )
    manifest = {
        "schema_version": 1,
...
def test_materialize_bundle_accepts_paired_adaptation_inputs(tmp_path):
    bundle, digest = _bundle(tmp_path, adaptation=True)
    output = tmp_path / "runtime"
    ...
    ).read_bytes() == b"train predictions"
    assert (output / "train/metadata.csv").read_text().startswith("spec_name")

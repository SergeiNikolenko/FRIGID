...
def test_extract_metrics_preserves_exact_metrics_and_builds_score():
    ...
    assert metrics["research_stage"] == "exact_retrieval"
    assert metrics["research_score"] == pytest.approx(3.24)
    assert metrics["tanimoto_top1"] == 0.0


def test_extract_metrics_promotes_validity_before_exact():
    metrics = MODULE.extract_metrics(
        {
            "Molecular metrics": {
                "Exact@1": {"last": 0.0},
                "Exact@10": {"last": 0.0},
                "Candidate return": {"last": 0.0},
                "Validity": {"last": 0.25},
                "Mass validity": {"last": 0.0},
                "Uniqueness": {"last": 0.0},
            }
        }
    )

    assert metrics["research_stage"] == "valid_decoding"
    assert metrics["research_score"] == pytest.approx(0.25)

...
| Optional M+1/M and M+2/M isotope token, enabled in training and disabled by default in inference | `marlin.isotopes`, `MarlinCollator`, `MarlinConditioner`, `--isotope-token` | Implemented. Training always emits the token, so omitting it at sampling time moves every forward pass off the training conditioning layout. A paired ablation on the warm start puts the constraint dead-end rate at `0.812` with no token, `0.625` with a zero-valued token, and `0.562` with the true ratios, which refutes the earlier rationale that an untrained placeholder would be worse than the missing slot. `--isotope-token omit\|zeros\|oracle` selects the sampling behaviour and is recorded as `isotope_token` in the run manifest; `omit` remains the default so existing provenance is unchanged, and `oracle` reads the target molecule and is a diagnostic only. Isotope-envelope calculation is inferred. |
...
| Grammar and forbidden-token masks at every reveal | `SafeGrammarMask` and sampler | Partial/inferred: full syntactic support is retained for contiguous prefixes and EOS is forbidden across an unresolved hole. Lexical support excludes partial tokens no SINGLE vocabulary token can finish: a bare `[` and its isotope digits never reach `add_atom`, so a saturated atom kept offering digits while no element could close the bracket, and 94.9% of the constraint dead ends recorded across the `fp-structure-probe` runs (500 of 527) end in `[` plus digits. Requiring a one-token completion closes that absorbing state without adding chemistry to the canonical path. Three measured limits are worth recording. The lookahead is depth one, so a state reachable only through two tokens is rejected even though it is completable: at prefix `C` the token `[ClH` is refused although `0` then `]` would close it. Support shrinks on real target paths as well, for instance `NS(=O` from 62 tokens to 8, but over 120 NPLIB1 targets and 6,802 gold decoding steps the gold tokenization never enters an incomplete-token state, so no gold step is ever rejected. The check costs about 2.5x wall clock per spectrum and, on its own, moved no headline metric: the oracle panel was unchanged only by cancellation across five rows, and the DreaMS panel regressed slightly, from validity 0.2109 to 0.2031. Other tokens are conservatively not grammar-pruned across holes because the paper does not define the required existential grammar state. Completed blocks still pass strict SAFE decoding. Chemical mass reachability is not folded into the syntax mask on the canonical path; `--mass-reachability-prune` enables it as an opt-in deviation and is recorded as `mass_reachability_prune` in the run manifest. It is no longer unmeasured. On the 32 spectrum `nplib1_val_micro32_v1` panel with oracle fingerprints and the step=10000 adaptation checkpoint, enabling it moves candidate return from 0.031 to 0.156, mass validity from 0.031 to 0.117 and constraint dead ends from 3.06 to 2.16 of 4, while `completed_validity` falls from 0.484 to 0.349: more branches survive to the end and a smaller share of the survivors are valid molecules. Admissibility was checked separately by walking the gold SAFE string of all 32 targets; reachability holds at every prefix, so the prune cannot remove the correct molecule from the search. A second opt-in deviation, `--forbid-isotope-tokens`, withholds support from the 479 vocabulary entries that open a bracket atom with a mass number, recorded as `forbid_isotope_tokens` in the run manifest. Admissibility is established by construction rather than by sampling: tokenizing all 7,144 adaptation targets uses none of these 479 entries, and the mass shell is defined on monoisotopic mass, so an isotopologue can only miss it. They are not idle in the failure set, appearing in 24% to 40% of the strings the decoder abandons. Measured over prefixes of real generated strings, mass-reachable support per position falls from 1,208 to 907 tokens, the mask runs 1.67x faster, and no prefix loses all support. The same measurement bounds what the mass prune itself contributes at mid-sequence positions, where it removes only 45 of 1,253 syntactically valid tokens; its effect is concentrated near the end of a sequence, where the remaining mass budget binds. A third opt-in deviation, `--restrict-organic-elements`, withholds support from tokens introducing an element outside CHNOPS and the halogens, recorded as `restrict_organic_elements`. The element set is declared from the assay rather than read off the answers, and checked afterwards: the vocabulary can write 123 distinct elements, 959 token entries fall outside the declared set, and the 7,144 targets between them use only C, N, O, P, S, F and Cl, so no gold token is affected. A token whose element cannot be resolved stays supported. Combined with the isotope ban this withholds 1,287 of 1,880 entries, mass-reachable support per position falls from 1,208 to 234, the mask runs 4.59x faster, and no probed prefix loses all support. Both switches were then measured end to end on the 32-spectrum panel against the step=30000 FRIGID warm-start checkpoint at 8 candidates: `exact_top1` holds at 0.0312 with the prune alone and with the isotope ban added, candidate return and uniqueness are identical at 0.2500 and 0.2321, validity moves from 0.2383 to 0.2539, and panel wall clock falls from 7.30 h to 4.19 h, a 1.74x speedup that matches the 1.67x measured on the mask alone. The isotope ban therefore buys throughput at no measured cost in quality; it is not an accuracy fix. |

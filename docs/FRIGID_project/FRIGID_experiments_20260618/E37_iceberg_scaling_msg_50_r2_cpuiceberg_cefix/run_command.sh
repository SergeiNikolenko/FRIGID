...
source .venv/bin/activate
{
  ...
  grep -n 'preserving the original key type' src/dlm/utils/masking_utils.py || true
  python - <<'PY'
...
PY
  python scripts/spec2mol_scaling.py \
    ...
    --output-dir repro_runs/20260622T_iceberg_scaling_msg_50_r2_cpuiceberg_cefix/results \
    --split test \
    --max-spectra 50 \
    ...
    --iceberg-cpu \
    --iceberg-results-dir repro_runs/20260622T_iceberg_scaling_msg_50_r2_cpuiceberg_cefix/iceberg_results \
    --batch-size 32 \
...
} 2>&1 | tee repro_runs/20260622T_iceberg_scaling_msg_50_r2_cpuiceberg_cefix/run.log

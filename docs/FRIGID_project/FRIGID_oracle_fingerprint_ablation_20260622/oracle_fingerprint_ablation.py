...
def add_extended_rank_metrics(result, predictions, target_inchi_key, target_fp, fp_bits, fp_radius):
    from dlm.utils.benchmark_utils import compute_morgan_fingerprint, compute_tanimoto_similarity

    pred_keys = [p.get("inchi_key") for p in predictions]
    exact_rank = None
    if target_inchi_key in pred_keys:
        exact_rank = pred_keys.index(target_inchi_key) + 1
    ...
    for pred in predictions:
        ...
        gt_similarities.append(compute_tanimoto_similarity(target_fp, mol_fp))

    result["num_predictions_full"] = len(predictions)
    result["exact_rank"] = exact_rank
    result["exact_match_top100"] = 1.0 if exact_rank is not None and exact_rank <= 100 else 0.0
    result["tanimoto_top100"] = max(gt_similarities[:100]) if gt_similarities else 0.0
...
def main() -> None:
    ...
    for ordinal, spec_name in enumerate(requested_names, start=1):
        ...
        add_extended_rank_metrics(result, predictions, target_inchi_key, target_fp, fp_bits, fp_radius)
        result["spec_name"] = spec.get_spec_name()
        result["fingerprint_source"] = "oracle_ground_truth"
        result["mist_tanimoto"] = 1.0
        ...
        print(
            ...
            f"oracle_top100={result['exact_match_top100']:.0f} "
            f"oracle_tan10={result['tanimoto_top10']:.3f} "
            f"rank={result['exact_rank']} "
    ...
    aggregate["exact_match_top100"] = float(np.mean([r.get("exact_match_top100", 0.0) for r in results]))
    aggregate["tanimoto_top100_mean"] = float(np.mean([r.get("tanimoto_top100", 0.0) for r in results]))
    exact_ranks = [r["exact_rank"] for r in results if r.get("exact_rank") is not None]
    aggregate["avg_exact_rank_for_hits"] = float(np.mean(exact_ranks)) if exact_ranks else None
    ...
    comparison = comparison.rename(
        columns={
            ...
            "exact_match_top100": "oracle_exact_match_top100",
            "exact_rank": "oracle_exact_rank",
            "tanimoto_top1": "oracle_tanimoto_top1",
            "tanimoto_top10": "oracle_tanimoto_top10",
            "tanimoto_top100": "oracle_tanimoto_top100",
        }
    )
